...
Stasi_Aris	{
	...
	*new { | server, addr, chan = 0 |
	...
	init { | argServer, argAddr, argChan = 0 |
		...
		addr =  argAddr ?? { NetAddr("192.168.1.13", 12345); }; //localhost, oF port		
...
//		this.seashellGUI;	//call seashell GUI
	...
	soundGUI	{
		...
		wSound.bounds_(Rect(0,0,330,980));
		...
		//numSoundSketches
		numSoundSketches=EZSlider(wSound, 300@20, "numSoundSketches ", ControlSpec(1, 999, \lin, 1, 5), numberWidth:50,layout:\horz, initVal:99, labelWidth: 120);
		...
		//maxAmp
		maxAmp=EZSlider(wSound, 300@20, "maxAmp ", numberWidth:50,layout:\horz, initVal:0.15, labelWidth: 100);
	...
	mainGUI	{
		...
		window.bounds_(Rect(330,0,330,980));
		...
		feedbackSpeedOrient = Slider2D(window, Rect(20, 20,80, 80))
						.action_({|sl|
							...
							addr.sendMsg("/feedbackSpeedX", linlin(sl.x, 0, 1, -1, 1));
							addr.sendMsg("/feedbackSpeedY", linlin(sl.y, 0, 1, -1, 1));
						});
		...
		Button(window, Rect(20,20,300,20))
				.states_([
					["view mask beat", Color.black, Color.white],
					["stop mask beat", Color.white, Color.black],
				])
