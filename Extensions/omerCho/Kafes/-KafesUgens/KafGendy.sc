...
KafGendy : UGen { 
	...
	*ar { arg  att= 0.1, dec = 1.8, sus = 2.1, rls = 2.5, mul = 1.0, loop = 0, freq = #[430, 419, 546, 789], freqrlp1 = 400, freqrlp2 = 350, out = 0;
		
		^SynthDef(\gendy01,{|i, k, gate = 1|
			var ses, env, mix, rlpf1, rlpf2, norm, lag;
			
			
			env = EnvGen.ar(Env.new([0, 1.0, 1.4, 0.8, 0], [att, dec, sus, rls], [1, -3, -1]), doneAction:2);
			ses = Mix.new(Gendy3.ar(freq:freq));
			ses = RLPF.ar(ses, freqrlp1.abs/2.0**0.01.cos, 0.005pi);
			ses = RHPF.ar(ses, freqrlp2, 0.005pi);
			ses = Normalizer.ar(ses, 0.3);
