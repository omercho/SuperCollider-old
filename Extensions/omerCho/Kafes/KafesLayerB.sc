...
fork {
	...
	fork{
		...
		12.do {|i, fr|
			...
			(0.1 + (0.04*(i%15))).wait;	
			~kik01.brt_(9.5).playPV1(0.001, 1.1, 1, mul:0.05+(0.04*(i%9)), out: ~str); // tek
			(0.1 + (0.04*(i%15))).wait;
		};
...
~tol06.brt_(~kRB4 *1.23700).playGverbR(0.001, 2.1, 9.1, mul:0.5, room:55, rev:13.5, damp:0.71, out: ~strCok.choose);
...
fork{
	fork{
		~kPseq01 = Pseq([~kRC1, ~kRC3, ~kRC5, ~kRD1, ~kRC4, ~kRD1, ~kRC1, ~kRD1].rotate(3)/2, inf).asStream;
		~amp = Pseq([0.15, 0.25, 0.35].rotate/1.9, inf).asStream;
		24.do{|i, frq = 2240, dur = 0.35|
	...
	~tol06.brt_(~kRD4 *1.23700).playPV1R(0.001, 1.1, 16.1, mul:0.5, out: ~strTek.choose);
	1.0.wait;
	...
	~tol06.brt_(~kRD3 *1.23700).playPV1R(0.001, 1.1, 16.1, mul:0.5, out: ~strTek.choose);
	1.0.wait;
...
~tol06.brt_(~kRB1 *1.23700).playGverbR(0.001, 1.1, 9.1, mul:0.8, room:55, rev:23.5, damp:0.61, out: ~strTek.choose);
~gou02.brt_(~kRB4 *0.89000).playPV2(0.1, 3.1, 5.1, mul:0.1, out: ~strCok.choose);
...
fork{
	...
	fork{
		...
		1.5.wait;
		~kik01.brt_(5.5).playBuf(0.001, 1.1, 2, mul:0.6,  out: ~strTek.choose);
		0.5.wait;
		~kik01.brt_(2.5).playBuf(0.001, 1.1, 2, mul:0.4, out: ~strTek.choose);
		0.5.wait;
		~kik01.brt_(9.5).playBuf(0.001, 1.1, 2, mul:0.6,  out: ~strTek.choose);
	...
	fork{
		...
		8.do {|i|
			...
			~kik01.brt_(9.5).playGverb(0.001, 1.1, 1, mul: ~mul01.next, room:8.4, rev:1, damp:0.91, out: ~str);
			~dur01.next.wait;
...
~kik01.brt_(2.5).playPV4(0.001, 1.1, 1, mul:0.9, out: ~strCok.choose);
fork{
	...
	fork{
		3.do{
			...
			~kik01.brt_(7.5).playPV5(0.001, 1.1, 1, mul:amp, out: ~strCok.choose); // tek	
			dur.next.wait;
			
			
		};
		~kik01.brt_(9.5).playPV5(0.001, 1.1, 1, mul:0.9, out: ~strCok.choose); // tek	
	};
...
fork{
	...
	fork{
		~kPseq01 = Pseq([~kRD7, ~kRD6, ~kRD7, ~kRD4, ~kRD6, ~kRD5, ~kRD4, ~kRD3].reverse/6, inf).asStream;
		~dur = Pseq([0.25, 0.36, 0.47, 0.58, 0.69, 0.80, 0.91, 0.94]/8, inf).asStream;
		12.do{
...
fork{
	~amp = Pseq((0.005..0.55), 28).asStream;
	28.do{
		var dur = Pseq([0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25]*4, inf).asStream;
...
fork{
	...
	fork{
		~kik01.brt_(0.5).playGverb(0.001, 1.1, 1, mul: ~mul01.next, room:8.4, rev:1, damp:0.91, out: ~strCok.choose);
		~mul01 = Pseq([0.5, 0.4, 0.3, 0.2, 0.1, 0.09, 0.07, 0.04]*0.9, inf).asStream;
...
fork{		
	...
	fork{
		...
		8.do {|i|
			...
			~kik03.brt_(1.5).playGverb(0.001, 1.1, 1, mul: ~mul01.next, room:8.4, rev:1, damp:0.91, out: ~str);
			~dur01.next.wait;
		
		};
	};
	2.5.wait;
	~kik01.brt_(3.5).playGverb(0.001, 1.1, 1, mul: ~mul01.next, room:8.4, rev:1, damp:0.91, out: ~str);
